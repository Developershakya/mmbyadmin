...
function parseFlightGroup(item, fIdx, traceId, searchContext = {}, isReturn = false) {
  ...
  fares.forEach((fareItem, fareIdx) => {
    ...
    mappedFares.push({
      ...
      seatsLeft: fareItem.NoOfSeatAvailable || item.NoOfSeatAvailable || firstSeg.NoOfSeatAvailable || 3,
      noOfSeatAvailable: fareItem.NoOfSeatAvailable || item.NoOfSeatAvailable || firstSeg.NoOfSeatAvailable || 3,
      originAirportName: firstSeg.Origin?.AirportName || `${originName} Airport`,
      originTerminal: firstSeg.Origin?.Terminal ? `Terminal ${firstSeg.Origin.Terminal}` : 'Terminal 1D',
      destinationAirportName: lastSeg.Destination?.AirportName || `${destName} Airport`,
      destinationTerminal: lastSeg.Destination?.Terminal ? `Terminal ${lastSeg.Destination.Terminal}` : 'Terminal 1',
